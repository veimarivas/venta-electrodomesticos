{{--
    Un grupo de categoría del Stock Actual. Recibe $grupo (array), $nivel (int),
    $clave (string) para el estado de colapso y $colapsadas (array).
--}}
<section class="stock-grupo-categoria {{ ($colapsadas[$clave] ?? false) ? 'is-collapsed' : '' }}"
    @if (($nivel ?? 0) > 0) style="margin-left: 1.25rem" @endif>
    <header class="stock-grupo-cabecera">
        <div class="d-flex flex-wrap align-items-center gap-2 w-100">
            <button type="button" class="btn btn-sm btn-icon rounded-circle btn-ghost-secondary stock-grupo-toggle"
                ...
                aria-label="{{ ($colapsadas[$clave] ?? false) ? 'Expandir' : 'Contraer' }} {{ $grupo['categoria']?->nombre ?? 'Sin categoría' }}">
                <i class="ri-arrow-down-s-line fs-18"></i>
            </button>

            <span class="stock-grupo-icono">
                <i class="{{ $grupo['categoria'] ? 'ri-folder-2-fill' : 'ri-error-warning-line' }}"></i>
            </span>

            <h6 class="mb-0 flex-grow-1 text-truncate">
                @if (($nivel ?? 0) > 0)
                    <span class="fw-normal me-1 stock-text-muted">{{ str_repeat('· ', $nivel) }}</span>
                @endif
                {{ $grupo['categoria']?->nombre ?? 'Sin categoría' }}
            </h6>

            <span class="stock-chip" title="Productos en esta rama">
            ...
            <div class="stock-salud">
                <div class="d-flex flex-wrap justify-content-between gap-2 align-items-center">
                    ...
                    <small class="stock-salud-leyenda">
                        <span class="stock-text-accent"><i class="ri-checkbox-blank-circle-fill fs-10 align-middle me-1"></i>{{ $sanos }} sanos</span>
    ...
    @unless ($colapsadas[$clave] ?? false)
                    ...
                    <tbody>
                        @foreach ($grupo['productos'] as $producto)
                            ...
                            <tr class="stock-producto-fila">
                                ...
                                <td>
                                    @if ($producto->marca)
                                        <span class="stock-text-muted">{{ $producto->marca->nombre }}</span>
                                    @else
        ...
        @foreach ($grupo['hijos'] as $hijo)
            @include('livewire.stock.partials.categoria-grupo', [
                'grupo' => $hijo,
                'nivel' => ($nivel ?? 0) + 1,
                'clave' => $hijo['categoria'] ? 'cat-'.$hijo['categoria']->id : 'cat-sin',
