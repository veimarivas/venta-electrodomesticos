...
--}}
<section class="stock-grupo stock-grupo--categoria {{ ($colapsadas[$clave] ?? false) ? 'is-collapsed' : '' }}"
    @if (($nivel ?? 0) > 0) style="margin-left: {{ min($nivel, 3) * 1.1 }}rem" @endif>
    <header class="stock-grupo-cabecera">
        <div class="stock-grupo-fila">
            ...
            <div class="stock-grupo-nombre">
                <h6 class="stock-grupo-titulo">
                    {{ $grupo['categoria']?->nombre ?? 'Sin categoría' }}
                </h6>
                @if (($nivel ?? 0) > 0)
                    <span class="stock-grupo-nivel">Subcategoría</span>
                @endif
            </div>
                ...
                <table class="table table-hover align-middle mb-0 stock-tabla">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" class="stock-col-secundaria">Marca</th>
                    ...
                    <tbody>
                        @foreach ($grupo['productos'] as $producto)
                            ...
                            <tr class="stock-producto-fila">
                                ...
                                <td data-label="Marca">
                                    <span class="stock-celda-texto">{{ $producto->marca?->nombre ?? '—' }}</span>
                                </td>
