...
/**
 * Genera el código correlativo de los trabajadores con formato COD-0001.
 *
 * El correlativo se calcula sobre el máximo existente incluyendo los
 * registros archivados (softDeletes): reutilizar el código de un trabajador
 * dado de baja rompería el histórico y chocaría con el índice único.
 */
class GeneradorCodigoTrabajador
{
    public const PREFIJO = 'COD-';
    ...
    /**
     * Crea el trabajador asignándole el siguiente código libre.
     *
     ...
     */
    public function crearCon(array $datos): Trabajador
    {
        for ($intento = 1; $intento <= self::INTENTOS; $intento++) {
            try {
                return DB::transaction(fn (): Trabajador => Trabajador::create([
                    ...$datos,
        ...
        throw new \RuntimeException('No se pudo generar un código de trabajador.');
    }

    /**
     * Mayor correlativo usado hasta ahora, 0 si aún no hay trabajadores.
     */
    private function ultimoCorrelativo(): int
    {
        $maximo = Trabajador::withTrashed()
    ...
    {
        return str_contains($e->getMessage(), 'trabajadores_codigo_unique')
